...
// --- 连接信息 ---
// MySQL DSN
$dsn = "mysql:host=localhost;dbname=pdo_mysql;charset=utf8mb4";
// 数据库用户名
$username = "root";
...
try {
    ...
    $sql = "SELECT id, studentName, studentAge, studentGrade FROM main WHERE studentGrade = 'A'";
    ...
    foreach ($stmt as $row) {
        echo "<li>ID: {$row['id']}, Name:" . htmlspecialchars($row['studentName']) . ", Age: {$row['studentAge']}, Grade: {$row['studentGrade']}</li>";
    }
    echo "</ul>";
} catch (PDOException $e) {
...
try {
    // 这个 UPDATE 也是固定的
    $sql = "UPDATE main SET studentGrade = 'A' WHERE studentGrade = 'B'";
